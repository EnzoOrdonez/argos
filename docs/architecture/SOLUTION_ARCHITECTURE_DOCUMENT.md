...
3. Notification service sends email to all configured recipients with: alert summary, LLM analysis, proposed actions, **JWT-signed buttons** for "Approve isolation" / "Reject — false positive".
...
Test pyramid mandatory for components in critical detection / decision paths:

- **Unit tests** (per module, ~70% coverage target):
...
3. **Multi-channel notifications** (Slack, Teams, Telegram, SMS, PagerDuty). Foundation laid by ADR-0005, implementations deferred.
...
1. **Should Layer 4 LLM be on the critical containment path or analyst-only?** Current decision: enrichment only, not containment trigger. Rationale: avoid LLM hallucination causing wrongful isolation. Revisit at Gate 2.
...
5. **Tier threshold calibration:** boundaries between T0/T1/T2/T3 (currently 0.95, 0.80, 0.60, 0.40) are placeholders. Calibrate with empirical false-positive / false-negative data in Week 9.
...
*This document is a living artifact and is updated whenever an Architecture Decision Record (ADR) is accepted or a design changes. Last updated: Week 1.*
